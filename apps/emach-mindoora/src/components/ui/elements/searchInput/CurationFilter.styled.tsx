import { styled } from "styled-components";
import { size } from "~/ui/components/foundations/breakpoints/device";

const SearchWrapper = styled.div`
  display: flex;
  flex-direction: row;
  width: 100%;
  justify-content: space-between;
  align-items: center;
  gap: 20px;

  @media (max-width: ${size.md}) {
    flex-direction: column;
  }

  img {
    margin-right: 1px;
  }

  input {
    font-family: inherit;
    font-size: inherit;
    background-color: #f4f2f2;
    border: none;
    color: #000;
    padding: 0.8rem 1rem;
    border-radius: 20px;
    width: 12em;
    transition: all ease-in-out 0.5s;
    margin-right: -2rem;
  }

  select {
    padding: 0.7rem 1rem;
    border-radius: 10px;
    background-color: #7d59a5;
    color: #fff;
    padding-right: 10px;
    padding-left: 10px;
    display: flex;
    justify-content: center
    align-items: center;
    font-size: 1rem;
    font-weight: bold;
  }
`;

const PlayAloneContainer = styled.label`
  --UnChacked-color: hsl(0, 0%, 10%);
  --chacked-color: #4d297b;
  --font-color: white;
  --chacked-font-color: var(--font-color);
  --icon-size: 1.5em;
  --anim-time: 0.2s;
  --anim-scale: 0.1;
  --base-radius: 0.8em;

  display: flex;
  align-items: center;
  position: relative;
  cursor: pointer;
  font-size: 20px;
  user-select: none;
  fill: var(--font-color);
  color: var(--font-color);

  input {
    display: none;
  }

  .checkmark {
    background: var(--UnChacked-color);
    border-radius: var(--base-radius);

    display: flex;
    padding: 0.5em;
  }
  .icon {
    width: var(--icon-size);
    height: auto;
    filter: drop-shadow(0px 2px var(--base-radius) rgba(0, 0, 0, 0.25));
  }
  .name {
    margin: 0 0.25em;
  }
  .Yes {
    width: 0;
  }
  .name.Yes {
    display: none;
  }

  /* action custom checkbox */
  hover .checkmark,
  hover .icon,
  hover .name {
    transform: scale(calc(1 + var(--anim-scale)));
  }

  active .checkmark,
  active .icon,
  active .name {
    transform: scale(calc(1 - var(--anim-scale) / 2));
    border-radius: calc(var(--base-radius) * 2);
  }
  .checkmark::before {
    content: "";
    opacity: 0.5;
    transform: scale(1);
    border-radius: var(--base-radius);
    position: absolute;
    box-sizing: border-box;
    left: 0;
    top: 0;
    height: 100%;
    width: 100%;
  }
  .checkmark:hover:before {
    background-color: hsla(0, 0%, 50%, 0.2);
  }
  input:checked + .checkmark:before {
    animation: boon calc(var(--anim-time)) ease;
    animation-delay: calc(var(--anim-time) / 2);
  }

  /* When the checkbox is checked*/
  input:checked + .checkmark {
    --UnChacked-color: var(--chacked-color);
    fill: var(--chacked-font-color);
    color: var(--chacked-font-color);
  }

  input:checked ~ .checkmark .No {
    width: 0;
  }
  input:checked ~ .checkmark .name.No {
    display: none;
  }

  input:checked ~ .checkmark .Yes {
    width: var(--icon-size);
  }
  input:checked ~ .checkmark .name.Yes {
    width: auto;
    display: unset;
  }

  /*Animation*/

  .checkmark,
  .checkmark:after,
  .icon,
  .checkmark .name {
    transition: all var(--anim-time);
  }

  /*Unuse*/
  @keyframes icon-rot {
    50% {
      transform: rotateZ(180deg) scale(calc(1 - var(--anim-scale)));
      border-radius: 1em;
    }
    to {
      transform: rotate(360deg);
      border-radius: var(--base-radius);
    }
  }
  /*Unuse*/
  @keyframes boo {
    80% {
      transform: scale(1.4);
    }
    99% {
      transform: scale(1.7);
      border: 2px solid var(--UnChacked-color);
    }
    to {
      transform: scale(0);
    }
  }
`;

export const Styled = {
  SearchWrapper,
  PlayAloneContainer,
};
