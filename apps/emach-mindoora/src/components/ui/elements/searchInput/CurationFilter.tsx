...
import { Styled } from "./CurationFilter.styled";
...
export const CurationFilter = ({
  onStringChange,
  stringValue,
  pathName
}: {
  onStringChange: (searchTerm: string) => void;
  stringValue: string;
  pathName: string
}) => {
  const [isSoloPlay, setIsSoloPlay] = useRecoilState(isSoloPlayRecoilState);
  const router = useRouter();
  const isPortals = pathName === "/portals";
  ...
  const handleSearch = (e: any) => {
    ...
    onStringChange(capitalizeFirstLetter(stringValue));
    router.push({
      pathname: pathName,
      query: { category: capitalizeFirstLetter(stringValue) },
    });
  };
  ...
  return (
    <Styled.SearchWrapper>
      ...
      <div className="flex">
        <Image src="/assets/search.png" alt="search" width={50} height={50} />
        <input
          ...
          required
        />
        {/* just category is added. More will be added on demand. */}
        {stringValue !== "" && isPortals && (
          <GenericButton
            variant="filled"
      ...
      <div>
        <Styled.PlayAloneContainer>
          ...
          <div className="checkmark">
            ...
            <p className="No name">Multiplayer</p>
            <svg
              viewBox="0 0 16 16"
              className="icon Yes"
              xmlns="http://www.w3.org/2000/svg"
