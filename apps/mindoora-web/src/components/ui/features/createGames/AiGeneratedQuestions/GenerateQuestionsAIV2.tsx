...
import { LanguageOptions, NumOptions } from "../category.data";
...
export const GenerateQuestionsAIV2 = () => {
  ...
  const generateQuestions = async (e) => {
    ...
    const nTopicSylables = inputTopic.split(" ").length;
    if (nTopicSylables > 3) {
      toast.error("Topic is more then 3 words.");
      return;
    }

    const api = await apiSetup();
    const baseURL = `https://ai.bulkbdsms.com/questions/from_text_g`;
    const params = {
      topic: inputTopic,
      lan: Language,
      max: numQuestions,
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        toast.success("Got Questions Successfully.");
        api
  ...
  return (
    <ModalWrapper>
      ...
      <div className="flex gap-2">
        ...
        >
          {questionState.length === 0 ? "GENERATE" : "ADD MORE"}
        </GenericButton>
        <GenericButton
          backgroundcolor="#FF6F40"
          textcolor="#fff"
